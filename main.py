...
def test_P():
    ...
    ax.set_xlabel("Кол-во людей в очереди")
    ax.set_ylabel("Доля времени")
    plt.legend()
    plt.savefig(f"pics/MU_{MU}_L_{LAMBDA}_SIZE_{SIZE}_CAP_{MAX_CAPACITY}_P.png")     
...
def test_L():
    ...
    with open(f"pics/MU_{MU}_L_{LAMBDA}_SIZE_{SIZE}_CAP_{MAX_CAPACITY}_L.txt", "w") as f:
        f.write(f"Test L: {L} - {np.mean(queue)}")
...
def test_W():
    ...
    fig, ax = plt.subplots()
    plt.title(f"Среднее время ожидания")
    ...
    ax.set_ylabel("Среднее время ожидания W")
    plt.legend()
    plt.savefig(f"pics/MU_{MU}_L_{LAMBDA}_SIZE_{SIZE}_CAP_{MAX_CAPACITY}_W.png")    
